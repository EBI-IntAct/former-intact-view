...
this.reset = function () {
    this.lastTakeTime = this.lastDeltaTime = this.startTime = new Date().getTime();
    this.stops = [];
};
this.takeTime = function (info) {
    var now = new Date().getTime();
    this.stops[this.stopCnt] = [];
    this.stops[this.stopCnt]['INFO'] = info;
    this.stops[this.stopCnt]['TOT'] = now - this.startTime;
    this.stops[this.stopCnt]['DELTA'] = now - this.lastTakeTime;
...
this.getDelta = function () {
    var now = new Date().getTime();
    var delta = now - this.lastDeltaTime;
...
this.toHtml = function(title) {
    var ret = [];
    var ii = 0;
    if (title != '') ret[ii++] = "<B>" + title + "</B><br>";
    this._weightIt();
    ret[ii++] = '<table cellspacing="0" cellpadding="2">';
    ret[ii++] = '<tr>';
    ret[ii++] = '<th bgcolor="Aqua">Nr.</th>';
    ret[ii++] = '<th bgcolor="Silver">INFO</th>';
    ret[ii++] = '<th bgcolor="Aqua">DELTA<br>(ms)</th>';
    ret[ii++] = '<th bgcolor="Silver">TOT<br>(ms)</th>';
    ret[ii++] = '<th bgcolor="Aqua">-</th>';
    ret[ii++] = '</td>';
    var stopSize = this.stops.length;
