var reactomeAnalysis = function(btn, query, negative, spoke, projection) {
    ...
    if (projection) {
        reactomeCorsURI = 'https://www.reactome.org/AnalysisService/identifiers/url/projection?pageSize=0&page=1';
    } else {
        reactomeCorsURI = 'https://www.reactome.org/AnalysisService/identifiers/url?pageSize=0&page=1';
    }

    // ToDo: make this URI configurable to support local testing. perhaps pass in as argument
    var dataURI = "https://www.ebi.ac.uk/intact/list?query="+ query+"&negative="+negative+"&spoke="+spoke;
        ...
        .done(function(data) {
            ...
            if (data.pathwaysFound > 0) {// results in Reactome
                btn3.get(0).onclick = function(){
                    window.open("https://www.reactome.org/PathwayBrowser/#DTAB=AN&ANALYSIS=" + data.summary.token, "_blank");
                };
...
var reactomeAnalysisList = function() {
    ...
    var reactomeCorsURI = 'https://www.reactome.org/AnalysisService/identifiers/?pageSize=0&page=1&sortBy=ENTITIES_PVALUE&order=ASC&resource=TOTAL';
